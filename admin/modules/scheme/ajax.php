        ...
        case "add_scheme":
            ...
            if( !empty( $schemes->adp_number ) ) {
                if( !empty( $schemes->id ) ) {
                    ...
                    doquery("insert into schemes(sector_id, taluka_id, category_id, adp_number, approval_year, project_description, completion_date, estim_cost, actual_expenditure, estim_expenditure, capital, electric, rev) values('".slash($schemes->sector_id)."', '".slash($schemes->taluka_id)."', '".slash($schemes->category_id)."', '".slash($schemes->adp_number)."', '".slash($schemes->approval_year)."', '".slash($schemes->project_description)."', '".date_dbconvert($schemes->completion_date)."', '".slash($schemes->estim_cost)."', '".slash($schemes->actual_expenditure)."', '".slash($schemes->estim_expenditure)."', '".slash($schemes->capital)."', '".slash($schemes->electric)."', '".slash($schemes->rev)."')", $dblink);
                    $id = inserted_id();
